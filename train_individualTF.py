...
from utils.data_process import data_process_trans, data_process_mean_std, data_process_delta, get_data
from utils.vis_utils import plt_img_log_new
from utils.number_loader import NumberLoader
...
def main():
    ...
    dual_data = get_data(data_path='./data/002-chen-04-dualarmstirfry')
    data_mean, data_std = data_process_mean_std(data=dual_data, aug=False)
    data_l, data_r = np.array(dual_data[:7]), np.array(dual_data[7:])
    data_l = ((data_l.transpose() - data_mean) / data_std).transpose()
    data_r = ((data_r.transpose() - data_mean) / data_std).transpose()

    val_dual_data = get_data(data_path='./data/002-chen-04-dualarmstirfry')
    val_dual_data = data_process_trans(data=val_dual_data, all_rate=0.8)
    # val_dual_data = get_data(data_path='./data/002-chen-03-dualarmstirfry', trunc_at=200)
    val_data_mean, val_data_std = data_process_mean_std(data=val_dual_data, aug=False)
    val_data_l, val_data_r = np.array(val_dual_data[:7]), np.array(val_dual_data[7:])
    val_data_l = ((val_data_l.transpose() - val_data_mean) / val_data_std).transpose()
    val_data_r = ((val_data_r.transpose() - val_data_mean) / val_data_std).transpose()

    test_dual_data = get_data(data_path='./data/002-chen-04-dualarmstirfry')
    test_dual_data = data_process_trans(data=test_dual_data, all_rate=1)
    test_data_l, test_data_r = np.array(test_dual_data[:7]), np.array(test_dual_data[7:])
    test_data_l = ((test_data_l.transpose() - data_mean) / data_std).transpose()
    test_data_r = ((test_data_r.transpose() - data_mean) / data_std).transpose()
    ...
    test_inp = test_data_l.transpose()  # (1226, pose_dim)
    test_tgt = test_data_r.transpose()  # (1226, pose_dim)

    train_dataset = NumberLoader(inp, tgt, inp_len=sequence_len, out_len=sequence_len, D3=D3, pose_dim=pose_dim)
    # train_len = int(len(dataset) * 0.7)
    # val_len = len(dataset) - train_len
    # train_set, val_set = random_split(dataset, [train_len, val_len])
    tr_dl = DataLoader(dataset, batch_size=batch_size, shuffle=True,
                       num_workers=1)  # x: (1059, sl, pd), y: (1059, sl, pd)
    val_dataset = NumberLoader(val_inp, val_tgt, inp_len=sequence_len, out_len=sequence_len, D3=D3, pose_dim=pose_dim)
    val_dl = DataLoader(val_dataset, batch_size=batch_size, shuffle=True,
                        num_workers=1)  # x: (118, sl, pd), y: (118, sl, pd)

    test_dataset = NumberLoader(test_inp, test_tgt, inp_len=sequence_len, out_len=sequence_len, D3=D3,
                                pose_dim=pose_dim)
    test_dl = DataLoader(test_dataset, batch_size=1, shuffle=True,
                         num_workers=1)  # x: (118, sl, pd), y: (118, sl, pd)

    model = individual_TF.IndividualTF(7, 7, 7, N=args.layers,
